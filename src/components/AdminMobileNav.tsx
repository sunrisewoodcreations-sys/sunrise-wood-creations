...
// Same categorized structure as the desktop sidebar (AdminSidebarNav),
// adapted to a slide-down panel instead of an always-visible column —
            ...
            {categories.map(category => {
              ...
              return (
                <div key={category.label}>
                  ...
                  {isOpen && (
                    <div className="pl-3 border-l border-white/10 ml-4 mb-1">
                      {category.links.map(link => (
                        <Link
                          key={link.href}
                          href={link.href}
                          onClick={() => setOpen(false)}
                          className="flex items-center px-3 py-3 rounded-lg text-white/75 active:bg-white/10 text-sm border-b border-white/5 last:border-0"
                        >
                          {link.label}
