    ...
    <form onSubmit={handleSubmit} className="bg-white border border-walnut/10 rounded-xl p-5 mb-6 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input required value={name} onChange={e => setName(e.target.value)} placeholder="36x18x18 Cedar planter box" className="w-full border border-walnut/15 rounded-md px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-walnut mb-1">Size / details</label>
          <input value={sizeDetails} onChange={e => setSizeDetails(e.target.value)} placeholder="36in x 18in x 18in" className="w-full border border-walnut/15 rounded-md px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-walnut mb-1">Price ($)</label>
          <input value={price} onChange={e => setPrice(e.target.value)} placeholder="55" className="w-full border border-walnut/15 rounded-md px-3 py-2 text-sm" />
        </div>
      </div>
