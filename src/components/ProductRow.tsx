...
export default function ProductRow({ product }: { product: Product }) {
  ...
  if (editing) {
    return (
      <tr className="border-t border-black/10 bg-cream/40">
        <td className="px-4 py-3">
          <input value={name} onChange={e => setName(e.target.value)} className="w-full border border-black/15 rounded-md px-2 py-1 text-sm" />
        </td>
        <td className="px-4 py-3">
          <select value={productType} onChange={e => setProductType(e.target.value)} className="w-full border border-black/15 rounded-md px-2 py-1 text-sm">
            {PRODUCT_TYPES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
          </select>
        </td>
        <td className="px-4 py-3">
          <input value={sizeDetails} onChange={e => setSizeDetails(e.target.value)} className="w-full border border-black/15 rounded-md px-2 py-1 text-sm" />
        </td>
        ...
        <td className="px-4 py-3 text-right whitespace-nowrap">
          ...
          <button onClick={() => setEditing(false)} className="text-black/50 text-xs">Cancel</button>
  ...
  return (
    <tr className="border-t border-black/10">
      <td className="px-4 py-3 text-black/70">{product.name}</td>
      ...
      <td className="px-4 py-3 text-right whitespace-nowrap">
        {confirmingDelete ? (
          <>
            ...
            <button onClick={() => setConfirmingDelete(false)} className="text-black/50 text-xs">Cancel</button>
          </>
        ) : (
          <>
            <button onClick={() => setEditing(true)} className="text-xs text-black hover:underline mr-3">Edit</button>
            <button onClick={() => setConfirmingDelete(true)} className="text-xs text-black hover:underline">Delete</button>
          </>
        )}
      </td>
    </tr>
