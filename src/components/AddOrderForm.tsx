    ...
    <form onSubmit={handleSubmit} className="bg-white border border-[#1E3A5F]/10 rounded-xl p-5 mb-6 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input value={price} onChange={e => setPrice(e.target.value)} placeholder="225" className="w-full border border-[#1E3A5F]/15 rounded-md px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-[#1E3A5F] mb-1">Qty</label>
          <input type="number" min="1" value={quantity} onChange={e => setQuantity(e.target.value)} className="w-full border border-[#1E3A5F]/15 rounded-md px-3 py-2 text-sm" />
        </div>
      </div>
