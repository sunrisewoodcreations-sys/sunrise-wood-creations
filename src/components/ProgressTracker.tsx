            ...
            {i > 0 && (
              <div
                className={`absolute top-4 -left-1/2 w-full h-[3px] ${
                  isDone || isCurrent ? "bg-sage" : "bg-black/10"
                }`}
              />
            )}
            <div
              ...
              ${!isDone && !isCurrent ? "bg-white border-black/10" : ""}`}
            >
              {isDone && "✓"}
              {isCurrent && <span className="w-3 h-3 rounded-full bg-ember" />}
            </div>
            <div className={`text-[11px] text-center mt-2 leading-tight max-w-[100px] ${isCurrent ? "text-ember font-semibold" : "text-black/60"}`}>
              {step.label}
            </div>
            {(isDone || isCurrent) && stamp && (
              <div className="text-[10px] text-center mt-1 leading-tight max-w-[100px] text-black/40 font-mono">
                {formatStamp(stamp)}
              </div>
            )}
          </div>
