  ...
  return (
    <div className="bg-white border border-black/10 rounded-xl p-5 mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label className="block text-xs font-semibold text-black mb-1">Describe the design</label>
          <textarea
            required
            value={prompt}
        ...
        <div>
          <label className="block text-xs font-semibold text-black mb-1">Reference image (optional)</label>
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*"
      ...
      {resultUrl && (
        <div className="mt-5 border-t border-black/10 pt-5">
          <p className="text-sm font-semibold text-black mb-2">Result:</p>
          <img src={resultUrl} alt="Generated design" className="max-w-full rounded-md border border-black/10 mb-3" />
          <a
            href={resultUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block border border-black text-black px-4 py-2 rounded-md text-sm font-semibold"
          >
            Download full size
          </a>
        </div>
